...
今天实现将 `模版解析` ,分析哪些模版依赖了当前$data里的数据，然后把数据和模版绑定，能在数据更新时触发模版更新

:::tip
回顾上文做到了将$data上的数据挂在到this上并和模版绑定,下面会实现解析模版后数据绑定渲染
:::

## 1.还是创建vue
引入模版绑定 `Observer` 和模版解析 `Compiler`函数并执行
