...
*/
    ...
    plugins: [
      ...
      new ProgressBarPlugin({
        format: `  :msg [:bar] ${require("chalk").green.bold(
          ":percent"
        )} (:elapsed s)`,
      }),
      //antd的momentjs的替代方案
...
默认依赖:`webpack`,`webpack-cli`,`webpack-dev-server(webpack本地服务器)`
| &nbsp; | 作用 |
| :------------: | :-: |
| style-loader | 编译 css 到 style 标签上 |
| mini-css-extract-plugin | css 插件，压缩 css |
| css-loader | 编译浏览器可以识别的 css |
| sass-loader | 编译 scss,sass 成 css |
| postcss-loader | 解决 css 兼容性 |
| postcss-preset-env | 寻找 packagejson 配置，通过配置加载指定的 css 兼容性样式 |
| postcss-flexbugs-fixes | 解决 flex 的 bug |
| postcss-normalize | PostCSS 标准化 |
| postcss-px2rem | 屏幕缩放自适应 |
| postcss-loader | 解决 css 兼容性 |
| sass-resources-loader | 配置公共 scss |
| thread-loader | 编译多线程|
| resolve-url-loader | 路径处理器 |
| html-webpack-plugin | html 打包模版 |
| antd-dayjs-webpack-plugin | antd 的 moment 的替代方案 |
| progress-bar-webpack-plugin | 打包或者运行时候显示进度条 |
| file-loader | 处理其他资源 |
| babel-loader| 编译 js |
| @babel/core(很重要哦) | babel 默认需要 |
| @babel/preset-env(预处理器)| 提前处理 |
| @babel/preset-react| react 语法编译器 |
| babel-plugin-import| 可以按需导入 antd |
| core-js| 按需导入 js,提升性能 |
